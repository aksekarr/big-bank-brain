"""Synthetic BIS HTML and mocked transport; no publisher archives or network."""
import json
import sys
import tempfile
import unittest
...
HTML = ('<nav>Navigation noise</nav><div class="text__component"><p>Outside noise</p></div>'
        '<article><h1>Title</h1><div class="text__component"><h3>Focus</h3><p>'+INTRO+
        '</p><p>'+BODY+'<span hidden>Hidden noise</span><script>Script noise</script></p>'
        '<footer>Footer noise</footer><table><tr><td>Table noise</td></tr></table>'
        '</div><aside><p>Related noise</p></aside></article>')

ITEM = {'title': 'Invented analysis', 'url': 'https://www.bis.org/publications/invented',
        'publication_date': '2026-09-09', 'institution': 'Bank for International Settlements',
        'source_name': 'BIS and FSI publications', 'excerpt': 'Invented RSS excerpt.'}
ROBOTS = ('User-agent: *\nAllow: /\nDisallow: /api/search', 'text/plain')


class BisReadTests(unittest.TestCase):
    def test_article_text_excludes_chrome_and_handles_inline_text(self):
        ...
        for noise in ['Navigation', 'Hidden', 'Script', 'Footer', 'Related', 'Title', 'Outside', 'Table']:
    ...
    def test_transient_callback_diagnostics_and_no_writes(self):
        ...
        self.assertEqual(consume.call_args.args,(ITEM,read.extract_article(HTML),'publication_page_html'))
    ...
    def test_bad_url_and_too_many_items_never_fetch(self):
        with patch.object(read,'fetch') as fetch:
            for items in [[dict(ITEM,url='https://www.bis.org/api/search')],[ITEM]*4]:
    ...
    def test_selection_uses_current_state_and_window_without_writing(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(items[0]['source_name'],'BIS and FSI publications')
