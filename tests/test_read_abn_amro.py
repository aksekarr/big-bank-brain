"""Synthetic ABN HTML and mocked transport; no publisher archives or network."""
import json
import sys
import tempfile
import unittest
...
PANEL = ' '.join(sorted(read.PANEL))
HTML = ('<main><nav>Navigation noise</nav><div class="'+PANEL+'"><h1>Title</h1><p>'+INTRO+
        '</p></div><div class="'+PANEL+'"><p class="emc-mb-2 emc-text-left">'+BODY+
        '<span hidden>Hidden noise</span><script>Script noise</script></p>'
        '<footer>Footer noise</footer></div><aside><p>Related noise</p></aside></main>')
ITEM = {'title': 'Invented analysis', 'url': 'https://www.abnamro.com/research/en/our-research/invented',
        'excerpt': 'Invented listing excerpt.'}
ROBOTS = ('User-agent: *\nAllow: /\nDisallow: /api/search', 'text/plain')


class AbnReadTests(unittest.TestCase):
    def test_article_text_excludes_chrome_and_handles_inline_text(self):
        ...
        for noise in ['Navigation', 'Hidden', 'Script', 'Footer', 'Related', 'Title']:
    ...
    def test_transient_callback_diagnostics_and_no_writes(self):
        ...
        self.assertEqual(consume.call_args.args,(ITEM,read.extract_article(HTML),'article_html'))
        ...
        self.assertFalse(result[0]['listing_fallback_used'])
    ...
    def test_denied_robots_and_failed_access_use_existing_excerpt_only(self):
        for replies in [[('User-agent: *\nDisallow: /','text/plain')],
            ...
            with patch.object(read,'fetch',side_effect=replies) as fetch, patch.object(read.time,'sleep'):
                results = read.run([ITEM,dict(ITEM,url=ITEM['url']+'-two')],consume)
            self.assertEqual(fetch.call_count,len(replies))
            self.assertTrue(all(r['listing_fallback_used'] for r in results))
    ...
    def test_bad_url_and_too_many_items_never_fetch(self):
        with patch.object(read,'fetch') as fetch:
            for items in [[dict(ITEM,url='https://www.abnamro.com/api/search')],[ITEM]*4]:
    ...
    def test_selection_uses_current_state_and_window_without_writing(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(items[0]['source_name'],'ABN AMRO Group Economics')
