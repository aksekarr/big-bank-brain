"""One-item BIS extraction; default metadata preview makes no network requests."""
import argparse
import fcntl
import json
import os
...
LEDGER = 'bis-ai-spike-ledger.json'
RESULTS = 'bis-ai-extractions.json'
# One schema, validator, client and budget implementation; only source framing differs.
INSTRUCTIONS = ai.INSTRUCTIONS.replace('BBVA article-page text', 'BIS research text').replace(
...
def run_spike(client, root=processing.ROOT, now=None, limit=1, url=None):
    if limit != 1:
        raise ai.SpikeError('BIS spike permits exactly one article per invocation')
    ...
    try:
        ...
        if ai.held_calls(ledger) >= ai.MAX_CALLS:
            raise ai.SpikeError('BIS three-call allowance exhausted; review required')
        path = root / RESULTS
        ...
        if not isinstance(saved, dict) or saved.get('version') != 1 or not isinstance(saved.get('items'), dict):
            raise ai.SpikeError('Invalid saved BIS extraction file')
...
def main(argv=None):
    ...
    try:
        ...
        print(f'BIS extraction stopped: {error}. Item remains unprocessed.', file=sys.stderr)
        return 1
    except Exception:
        print('BIS extraction stopped; no raw content or provider errors logged.', file=sys.stderr)
        return 1
