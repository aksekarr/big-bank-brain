## Canonical manual synthesis

Use `.venv/bin/python -B scripts/synthesise_candidate.py` for read-only offline
assembly/request/accounting diagnostics. A legitimate new input set may be submitted
only through an explicitly authorised `--live --limit 1` invocation. No scheduler,
verification call or publication follows automatically. `synthesise.py` remains the
unchanged historical runner and the source of the tested prompt/schema/parser.

Canonical accounting is `candidate-synthesis-ledger.json`, never the historical
`synthesis-spike-ledger.json`. SHA-256 covers the exact UTF-8 `input[0].content` string
constructed by the existing request helper. The packet is assembled once; coverage
and assembly diagnostics outside model input are deliberately outside that hash.
Stored source context comes from that same request input. A returned response consumes
that input hash, including refusal/invalid output; pending uncertainty blocks it.
Definite 401/429 releases eligibility for a later manual attempt without removing
history. No automatic retries or arbitrary lifetime experiment-call cap are added.

Request/model assumptions are unchanged: 20,000 serialized UTF-8 bytes, low reasoning,
4,000 output tokens and $0.10056 per held snapshot, calculated as
(20,000 + 1,024) × $2.50/million + 4,000 × $12/million. This is the established local
reservation assumption, not measured billing or a newly checked price. Explicit live
invocation is the manual spending boundary; daily/global allocation remains a later
product task. Future oversized input blocks rather than truncating or expanding limits.
...
parse/validation or failed replacement preserves the previous candidate. A failure
persisting the final ledger update after successful result replacement can leave a
stored result with an earlier ledger stage; its input remains consumed. Writes are
atomic individually, not a multi-file transaction or an fsync durability guarantee.

Canonical ledger eligibility does not infer product legitimacy from historical
experiment files. In particular, an empty canonical ledger is NOT permission to
regenerate the existing five-article input to escape the consumed verification SHA.
Historical results do not retain the complete synthesis request input, so this adapter
does not invent a historical canonical hash or migrate old attempts. The next live
run must use legitimate changed/new source input and explicit owner authorisation.
No timestamp/reordering operation is a sanctioned way to obtain a new opportunity.

The offline integration test exercises actual source adapters under mocks → stored
extractions/processed state → assembly → canonical synthesis mock → temporary
`synthesis-result.json` → canonical verifier offline input. It includes one optional
source failing. No real synthesis or verification state is generated by these tests.

## Canonical MVP verification (human review required)

After `synthesise_candidate.py` stores `synthesis-result.json`, use:
