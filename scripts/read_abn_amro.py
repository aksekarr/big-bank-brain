"""ABN AMRO public HTML READ spike; diagnostics only, no model calls or raw files."""
import argparse
import json
import re
import subprocess
...
ROBOTS = 'https://www.abnamro.com/robots.txt'
USER_AGENT = 'BigBankBrain/0.1 (ABN AMRO public research READ feasibility)'
VOID = {'area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr'}
PANEL = {'emc-max-w-[52.5rem]', 'emc-p-10', 'emc-bg-[--background-primary]', 'emc-rounded-xl'}
...
def select(root=ROOT, now=None, limit=3, *, url=None):
    ...
    for item in items.values():
        if item['source_name'] != 'ABN AMRO Group Economics':
            continue
    ...
    if url is not None:
        ...
        if not eligible:
            raise ValueError('Requested ABN URL is not eligible and unprocessed')
...
class ArticleText(HTMLParser):
    """Observed ABN article panels only; ignore author cards and related articles."""
    def __init__(self):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        panel = tag == 'div' and PANEL <= classes and any(t == 'main' for t, _ in self.stack)
        if panel:
            self.panels += 1
        flags = {'panel': panel, 'hidden': hidden,
                 'text': tag in {'p', 'li', 'h2', 'h3'},
                 'body': tag == 'p' and {'emc-mb-2', 'emc-text-left'} <= classes}
        if tag not in VOID:
...
def extract_article(html):
    ...
    if parser.panels != 2 or parser.closed != 2 or parser.malformed:
...
def run(items, on_text=None):
    ...
    for item in items:
        ...
        if (url.scheme != 'https' or url.netloc != 'www.abnamro.com'
                or not url.path.startswith('/research/en/our-research/')
                or url.query or url.fragment or re.search(r'[\s\\]', item['url'])):
            raise ValueError('Unexpected ABN article URL')
    ...
    for item in items:
        ...
        if not text:
            ...
            source = 'listing_excerpt' if text else 'none'
...
def main(argv=None):
    ...
    try:
        ...
        print('ABN READ stopped; no raw content logged.', file=sys.stderr)
        return 1
