"""Liberty Street public HTML READ spike; diagnostics only, no model calls or raw files."""
import argparse
import json
import re
import subprocess
...
ROBOTS = 'https://libertystreeteconomics.newyorkfed.org/robots.txt'
USER_AGENT = 'BigBankBrain/0.1 (Liberty Street public research READ feasibility)'
...
def select(root=ROOT, now=None, limit=3, *, url=None):
    ...
    for item in items.values():
        if item['source_name'] != 'Liberty Street Economics':
            continue
...
class ArticleText(HTMLParser):
    """Observed Liberty Street text components within the publication article only."""
    def __init__(self):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        hidden = (tag in {'nav', 'footer', 'header', 'aside', 'script', 'style', 'form',
                  ...
                  or bool(classes & {'cookie', 'cookie-banner', 'menu', 'ts-blog-article-author', 'is-style-bio-contact', 'cite-container', 'is-style-disclaimer', 'frbny-related', 'sharedaddy', 'newsletter', 'signup', 'footnotes', 'wp-block-footnotes'})
                  or bool(re.search(r'display\s*:\s*none|visibility\s*:\s*hidden', attrs.get('style', ''), re.I)))
        panel = tag == 'div' and 'ts-article-text' in classes and any(t == 'main' for t, _ in self.stack)
        if panel:
            self.panels += 1
...
def run(items, on_text=None):
    ...
    for item in items:
        ...
        if (url.scheme != 'https' or url.netloc != 'libertystreeteconomics.newyorkfed.org'
                or not re.fullmatch(r'/\d{4}/\d{2}/[^/]+/', url.path)
                or url.path.lower().endswith('.pdf') or url.query or url.fragment or re.search(r'[\s\\]', item['url'])):
            raise ValueError('Unexpected Liberty Street article URL')
...
def main(argv=None):
    ...
    try:
        ...
        print('Liberty Street READ stopped; no raw content logged.', file=sys.stderr)
        return 1
